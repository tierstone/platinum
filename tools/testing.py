...
MODE_RULES = {
    ...
    "fd-invalid": {
        "build_args": (),
        "required": ("sched ok", "user init", "fdinvalid ok"),
        "forbidden": ("trap ", "fail", "exit fail", "user as fail", "user elf fail", "tss stack fail", "!"),
    },
    "fd-read": {
        "build_args": (),
    ...
    "fd-closed": {
        "build_args": (),
        "required": ("sched ok", "user init", "fdclosed ok"),
        "forbidden": ("trap ", "fail", "exit fail", "user as fail", "user elf fail", "tss stack fail", "!"),
    },
    "open-read": {
        "build_args": (),
    ...
    "open-invalid": {
        "build_args": (),
        "required": ("sched ok", "user init", "openinvalid ok"),
        "forbidden": ("trap ", "fail", "exit fail", "user as fail", "user elf fail", "tss stack fail", "!"),
    },
    "exec-elf": {
        "build_args": ("--user-program", "pulse"),
    ...
    "exec-bad-elf2": {
        "build_args": (),
        "required": ("sched ok", "user init", "execbadelf2 ok"),
        "forbidden": ("trap ", "fail", "exit fail", "user as fail", "user elf fail", "tss stack fail", "!"),
    },
    "exec-bad-paths": {
        "build_args": (),
        "required": ("sched ok", "user init", "execbadpaths ok"),
        "forbidden": ("trap ", "fail", "exit fail", "user as fail", "user elf fail", "tss stack fail", "!"),
    },
    "exec-second": {
        "build_args": (),
        "required": ("sched ok", "user init", "pulse2 ok"),
        "forbidden": ("trap ", "fail", "exit fail", "user as fail", "user elf fail", "tss stack fail", "!"),
    },
}
...
def verify(mode: str, loops: int, timeout: float) -> int:
    ...
    if mode == "all":
        modes = ("off", "c", "elf", "elf-pulse", "yield-stress", "bad-syscall", "bad-elf", "bad-bootstrap", "fd-write", "fd-invalid", "fd-read", "rw-invalid", "fd-closed", "open-read", "open-flags", "open-invalid", "exec-elf", "dup-full", "bad-pointers", "exec-loop", "exec-bad-loop", "exec-transfer-fail", "exec-registry", "exec-paths", "exec-root", "exec-noent", "exec-nonexec", "exec-bad-elf2", "exec-bad-paths", "exec-second")
    else:
        modes = (mode,)
