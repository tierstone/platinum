# Platinum (/p/OS)
64-bit OS written in C. Runs exclusively on QEMU x86_64. GPLv3.

## Stack

| | |
|---|---|
| **Lang** | C99/C11 (Strict) |
| **Compiler** | Clang/LLVM |
| **LibC** | musl |
| **License** | GPLv3 |
| **Target** | QEMU x86_64 (`q35`, OVMF) |
| **Drivers** | VirtIO only (Net, Block, Console, RNG) |

## AI Policy

**HOSTILE.** Local OSS LLMs may be used for *syntax lookup* or *boilerplate scaffolding* only. Zero tolerance for committing unreviewed AI output. If your code smells like it was generated by a probabilistic token predictor without human understanding, it will be rejected. We build systems, we don't prompt them.

## Design Philosophy

/p/OS is a curated synthesis of Unix lineage, rejecting dogma for utility. It combines **Linux**’s ELF/GNU compatibility for usability, **OpenBSD**’s secure-by-default privilege separation and code hygiene, **FreeBSD**’s userspace networking (`netd`) and declarative init (`pinit`), and **Plan 9**’s "everything is a file" simplicity (9P/VFS).

## Build & Run

**Prerequisites:** Clang/LLVM (`clang`, `lld-link`), Python 3, QEMU, OVMF (`edk2-ovmf`).

```bash
# Build the EFI binary
python3 build.py all

# Launch in QEMU
python3 boot.py
```

## Roadmap

### Phase 1: Kernel Base
- [x] UEFI entry — x86_64 COFF entry point (`boot.S`)
- [x] Early serial output — Legacy UART `0x3F8` driver (`serial.c`)
- [x] Build pipeline — Clang + `lld-link`, PE32+ output (`build.py`)
- [x] QEMU/OVMF launch script (`boot.py`)
- [ ] GDT, IDT setup
- [ ] Paging — 4KB pages, identity map
- [ ] Memory manager — Buddy allocator + Slab allocator
- [ ] VirtIO-console driver (early `printk`)
- [ ] Preemptive round-robin scheduler
- [ ] Syscalls: `read`, `write`, `open`, `close`, `fork`, `execve`, `exit`, `waitpid`
- [ ] ELF loader for static binaries

### Phase 2: Storage & Filesystem
- [ ] VirtIO-block driver
- [ ] VFS — inodes, dentries, file descriptors
- [ ] `/p/fs` — log-structured, copy-on-write, B-tree metadata
- [ ] `mkfs.pfs` utility
- [ ] Mount `/p/fs` as root

### Phase 3: Init & Shell
- [ ] `pinit` — PID 1, parse service structs, spawn processes
- [ ] `psup` — PID 2, supervise daemons, restart on crash
- [ ] `sh` — POSIX-compliant shell (pipes, redirects, variables, globbing)
- [ ] `login` — basic auth against `/etc/passwd`
- [ ] `getty` — spawn shell on VirtIO-console

### Phase 4: Coreutils
- [ ] `cat`, `echo`, `pwd`, `cd`, `env`, `export`
- [ ] `ls` (`-l`, `-a`, `-R`)
- [ ] `cp` (`-r`, `-v`), `mv`, `rm` (`-r`, `-f`), `mkdir` (`-p`)
- [ ] `chmod`, `chown`
- [ ] `grep` (`-r`, `-i`, `-v`)
- [ ] `sed` — basic substitution
- [ ] `awk` — minimal pattern scanning

### Phase 5: Networking
- [ ] VirtIO-net driver
- [ ] `netd` — userspace TCP/IP stack (ARP, IPv4, UDP, TCP)
- [ ] DHCP client in `netd`
- [ ] `ifconfig` / `ip`
- [ ] `ping` — ICMP echo
- [ ] `sshd` — port Dropbear or OpenSSH (musl-linked)
- [ ] `wget` / `curl` — minimal HTTP client

### Phase 6: Polish
- [ ] `logd` — syslog-compatible daemon
- [ ] `cron` — task scheduler
- [ ] `ps`, `top`, `kill`, `nice`
- [ ] `man` — manual page viewer
- [ ] Kernel size pass (`-Os`, LTO)
- [ ] Final cleanup and documentation

## License
GPLv3. See [`LICENSE`](LICENSE).
